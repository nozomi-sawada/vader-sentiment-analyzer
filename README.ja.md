...
### ▶ ブラウザですぐに使う: https://nozomi-sawada.github.io/vader-sentiment-analyzer/

インストール不要・登録不要・ファイルのダウンロード不要です。レキシコンは組み込み済みで、入力したテキストはすべて手元のブラウザ内で処理されます（サーバーには送信されません）。
...
1. **https://nozomi-sawada.github.io/vader-sentiment-analyzer/** を開く
2. 同梱のVADERレキシコンと絵文字レキシコンが自動で読み込まれます
3. テキストを入力して「分析する」をクリック（サンプル文をクリックすれば、そのまま結果が表示されます）

画面は日本語で表示されます。右上の **English** ボタンで英語表示に切り替えられます（選択は記憶されます）。

### 方法2: 手元で動かす

1. このリポジトリをダウンロード（「Code」→「Download ZIP」を展開）またはクローンする
2. `index.html` をブラウザで開く
3. テキストを入力して「分析する」をクリック

> [!NOTE]
> レキシコンの自動読み込みは、HTTP経由でページを開いた場合（上記の公開ページ、またはローカルサーバー）に動作します。`index.html` をファイルとして直接開いた場合は、画面のボタンからレキシコンファイル（`third_party/vaderSentiment/` にあります）を手動で読み込んでください。
