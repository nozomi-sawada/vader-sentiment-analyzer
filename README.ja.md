...
1. `index.html` をブラウザで開く（`vader.js` と `app.js` を同じフォルダに置いてください）
2. レキシコンファイルをアップロード
...
分析エンジン（`vader.js`）は、本家Python実装（[vaderSentiment 3.3.2](https://github.com/cjhutto/vaderSentiment)）を一行ずつ忠実に移植したものです。出力が本家と一致することは、自動化されたゴールデンテスト（後述の「テスト」参照）で検証されています：
